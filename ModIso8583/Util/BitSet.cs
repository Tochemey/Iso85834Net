...
namespace ModIso8583.Util
{
    ...
    public class BitSet : ICloneable
    {
        ...
        {
            var max = Math.Min(bits.Length,
                bs.bits.Length);
            int i;
            for (i = 0; i < max; ++i) bits[i] &= bs.bits[i];
        ...
        {
            var i = Math.Min(bits.Length,
                bs.bits.Length);
            while (--i >= 0) bits[i] &= ~bs.bits[i];
        }
        ...
        public void Clear(int from,
            int to)
        {
            if (from < 0 || from > to) throw new ArgumentOutOfRangeException();
            if (from == to) return;
            ...
            for (var i = (int) lo_offset + 1; i < hi_offset; i++) bits[i] = 0;
        }
        ...
        {
            if (!(obj.GetType() == typeof(BitSet))) return false;
            var bs = (BitSet) obj;
            var max = Math.Min(bits.Length,
                bs.bits.Length);
            int i;
            for (i = 0; i < max; ++i) if (bits[i] != bs.bits[i]) return false;
            // If one is larger, check to make sure all extra bits are 0.
            for (var j = i; j < bits.Length; ++j) if (bits[j] != 0) return false;
            ...
            int to)
        {
            ...
            for (var i = (int) lo_offset + 1; i < hi_offset; i++) bits[i] ^= -1;
        }
        ...
        {
            var offset = pos >> 6;
            if (offset >= bits.Length) return false;
            ...
            int to)
        {
            ...
            var lo_offset = (uint) from >> 6;
            if (lo_offset >= bits.Length || to == from) return bs;

            var lo_bit = from & LONG_MASK;
            var hi_offset = (uint) to >> 6;
            if (lo_bit == 0)
            {
                var len = Math.Min(hi_offset - lo_offset + 1,
                    (uint) bits.Length - lo_offset);
                Array.Copy(bits,
                    lo_offset,
                    bs.bits,
                    0,
                    len);
                if (hi_offset < bits.Length) bs.bits[hi_offset - lo_offset] &= (1L << to) - 1;
                return bs;
            }

            var len2 = Math.Min(hi_offset,
                (uint) bits.Length - 1);
            var reverse = 64 - lo_bit;
            int i;
            for (i = 0; lo_offset < len2; lo_offset++, i++) bs.bits[i] = (bits[lo_offset] >> lo_bit) | (bits[lo_offset + 1] << reverse);
            if ((to & LONG_MASK) > lo_bit) bs.bits[i++] = bits[lo_offset] >> lo_bit;
            if (hi_offset < bits.Length) bs.bits[i - 1] &= (1L << (to - from)) - 1;
            return bs;
        }
        ...
        {
            long h = 1234;
            for (var i = bits.Length; i > 0;) h ^= i * bits[--i];
            return (int) ((h >> 32) ^ h);
        }
        ...
        public bool Intersects(BitSet set)
        {
            var i = Math.Min(bits.Length,
                set.bits.Length);
            while (--i >= 0) if ((bits[i] & set.bits[i]) != 0) return true;
            return false;
        }
        ...
        public bool IsEmpty()
        {
            for (var i = bits.Length - 1; i >= 0; i--) if (bits[i] != 0) return false;
            return true;
        }
        ...
        public void Or(BitSet bs)
        {
            ...
            for (var i = bs.bits.Length - 1; i >= 0; i--) bits[i] |= bs.bits[i];
        }
        ...
        {
            for (var i = other.bits.Length - 1; i >= 0; i--) if ((bits[i] & other.bits[i]) != other.bits[i]) return false;
